...
外壳比网页端多几个"能力"，也就多几条边界。这几条是刻意做的取舍，改主进程前请先读：

| 能力 | 风险 | 现在的处理 |
| --- | --- | --- |
| `http:request`（主进程代发请求） | 渲染进程一旦被注入脚本，它就是一条探测内网/本机的 SSRF 通道 | ① 只接受 `app://` 主框架发来的请求；② 目标 origin 必须是设置里填的那台 WebDAV 服务器（设置页逐键保存，正常同步零打扰）；③ 其他地址弹窗确认，同一 origin 每次运行只问一次 |
| 自签名证书放行 | 只按主机名记住"信任"，本次运行内被换成另一张证书会静默通过 | 按**证书指纹**记住；指纹变了重新询问（自建 WebDAV 的用户对"证书不受信任"弹窗早就麻木了，这一条很关键） |
| `will-download` | 按上次目录静默落盘 = 被入侵的服务器/私设包能直接往磁盘写文件 | 一律弹系统保存框，用户取消即取消下载 |
| `app://` 自定义协议 | 目录穿越读到渲染目录之外的文件 | 归一化后用 `RENDERER_DIR + path.sep` 做前缀校验（只比裸前缀会放行同级的 `renderer-xxx` 目录），拒绝时返回 403 |
| `storage.json` 里的 WebDAV 密码 | 明文落在应用数据目录，同机其它账户/备份/云同步都能读到 | 经 `safeStorage` 加密后落盘（`password` → `passwordEnc`），渲染进程侧契约不变。**代价**：密文绑定当前系统账户，把 `storage.json`（或绿色版整个目录）拷到另一台机器后需要重填一次应用密码 |
| 渲染进程本身 | 词条正文里的内联事件处理器 | `app://` 的 HTML 响应带 CSP（`script-src 'self'`），与 `web/public/_headers` 同源同策，仅多放行 `app:` |

主进程这几个函数在受限环境里也能自检（不需要 Electron 运行时）：
把 `main.js` 放进 `vm` 上下文、用桩替换 `require("electron")`，即可验证授权白名单、证书指纹、
路径穿越与凭据加解密。这类自检应当覆盖 `originAllowed` / `isAppFrame` / `serializedStore` 的改动。
